...
%% Plot the results
figure('position',[300 50 860 700])

% acceleration
subplot(3,1,1)
plot(a_ego.Values.time,a_ego.Values.Data,'r',...
     a_lead.Values.time,a_lead.Values.Data,'b', 'LineWidth' , 2)
grid on
ylim([-3.2,2.2])
...
plot(v_ego.Values.time,v_ego.Values.Data,'r',...
     ...
     v_lead.Values.time,v_set*ones(length(v_lead.Values.time),1),'k--', 'LineWidth' , 2)
grid on
...
plot(d_rel.Values.time,d_rel.Values.Data,'r',...
     d_rel.Values.time,d_safe,'b', 'LineWidth' , 2)
grid on
